...
public class JointView extends View {
    ...
    private Body createBall(){
        ...
        bodyDef.position.set(pixelsToMeters(width/2-300),
                pixelsToMeters(0));
//        bodyDef.angularVelocity = degreesToRadians(3);
     ...
     * 创建滑轮
     */
    private RevoluteJoint createPulleyJoint() {

        RevoluteJointDef def = new RevoluteJointDef();// 创建滑轮关节数据实例
// 初始化滑轮关节
        def.initialize(ball_1, ball_2,ball_1.getWorldCenter());
        def.enableLimit = true;
        def.maxMotorTorque = 10.0f;//马达
        def.motorSpeed = 0.0f;
        def.enableMotor = true;
        def.collideConnected=false;
        RevoluteJoint joint = (RevoluteJoint) world.createJoint(def);// 创建滑轮节点
